...
export default function AdminCommissionsPage() {
  ...
  const [filterStatus, setFilterStatus] = useState<string>(''); // e.g., 'Pending Payout'
  ...
  // Authentication check using the useAuth hook
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-gray-100 p-8">
        ...
        <div className="mb-6 flex space-x-4 rounded-md bg-white p-4 shadow">
          <div>
          ...
          {payoutError && (
            <p className="self-center text-sm text-red-500">Payout Error: {payoutError}</p>
          )}
        </div>
              ...
              {commissions.length > 0 ? (
                commissions.map(comm => (
                  <tr key={comm.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`inline-flex rounded-full px-2 text-xs leading-5 font-semibold ${getStatusColor(comm.status)}`}
                      >
                        {comm.status}
                      </span>
                    </td>
