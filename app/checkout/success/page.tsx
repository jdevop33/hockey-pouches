...
import { useAuth } from '@/context/AuthContext'; // Potentially use user for greeting

export default function CheckoutSuccessPage() {
  const searchParams = useSearchParams();
  const orderId = searchParams.get('orderId'); // Get orderId from URL query ?orderId=...
  const { user } = useAuth();
      ...
      <div className="bg-gray-100 min-h-[70vh] flex items-center justify-center py-12">
        <div className="mx-auto max-w-xl text-center bg-white p-8 sm:p-12 rounded-lg shadow-xl">
          ...
          <div className="mt-8 flex flex-col sm:flex-row justify-center gap-4">
            {orderId && user && (
                // Link to the customer's dashboard order view
                <Link 
