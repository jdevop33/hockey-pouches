...
const Navigation: React.FC = () => {
  ...
  if (user) {
    ...
    if (user.role === 'Admin') {
        dynamicNavItems.push({ href: '/admin/dashboard', label: 'Admin DB', roles: ['Admin'] });
    } else if (user.role === 'Distributor') {
         dynamicNavItems.push({ href: '/distributor/dashboard', label: 'Distributor DB', roles: ['Distributor'] });
    }
    // All logged-in users get generic dashboard and cart
    ...
    <footer className="bg-gray-800 py-8 text-white">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
          ...
          <div>
            ...
            <ul className="space-y-2">
              ...
              <li><Link href="/dashboard" className="text-gray-300 hover:text-white">My Account</Link></li> {/* Changed from /account */} 
            </ul>
          </div>
          <div>
            <h3 className="mb-4 text-lg font-semibold">Contact Us</h3>
            <p className="text-gray-300">Email: {process.env.NEXT_PUBLIC_CONTACT_EMAIL || 'info@nicotinetins.com'}</p>
        ...
        <div className="mt-8 border-t border-gray-700 pt-8 text-center text-gray-300">
          <p>&copy; {new Date().getFullYear()} Nicotine Tins by Hockey Puxx. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
