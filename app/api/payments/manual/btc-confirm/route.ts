...
export async function POST(request: NextRequest) {
  try {
    ...
    if (authResult.role !== 'Admin') {
      logger.warn('Non-admin attempted manual BTC confirmation', {
        userId: adminUserId,
    ...
    if (!validation.success) {
      logger.warn('Invalid BTC confirmation request body', {
    ...
    if (!result.success) {
      logger.error('Manual BTC payment confirmation failed', {
        orderId,
        transactionId,
    ...
    logger.info('Manual BTC payment confirmed successfully', {
      orderId,
      transactionId,
      adminUserId,
    });
