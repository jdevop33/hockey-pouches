...
export async function POST(request: NextRequest) {
  try {
    ...
    if (authResult.role !== 'Admin') {
      logger.warn('Non-admin attempted manual e-transfer confirmation', {
        userId: adminUserId,
    ...
    if (!validation.success) {
      logger.warn('Invalid e-transfer confirmation request body', {
    ...
    logger.info('Manual e-transfer payment confirmation request', {
      orderId,
      transactionId,
      adminUserId,
    });
    ...
    if (!result.success) {
      logger.error('Manual e-transfer payment confirmation failed', {
        orderId,
        transactionId,
    ...
    logger.info('Manual e-transfer payment confirmed successfully', {
      orderId,
      transactionId,
      adminUserId,
    });
