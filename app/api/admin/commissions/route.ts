...
import { commissionService, type ListCommissionsOptions } from '@/lib/services/commission-service';
import { isValidCommissionStatus, type CommissionStatus } from '@/lib/constants/commission-status';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (options.status && !isValidCommissionStatus(options.status)) {
      return NextResponse.json(
        { message: `Invalid status filter: ${options.status}` },
        { status: 400 }
      );
    }
