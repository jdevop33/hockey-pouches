...
export async function POST(request: Request) {
  try {
    ...
    if (!token) {
      return NextResponse.json({ verified: false, message: 'No authentication token' }, { status: 401 });
    }
    ...
    try {
      ...
      if (user.status !== 'Active') {
        return NextResponse.json({ verified: false, message: 'Account is not active' }, { status: 403 });
      }
      ...
      return NextResponse.json({ verified: false, message: 'Invalid or expired token' }, { status: 401 });
    }
