...
export async function confirmManualPayment(
    ...
    try {
        const result = await db.transaction(async (tx) => {
            const payment = await tx.query.payments.findFirst({ where: and( eq(schema.payments.orderId, orderId), eq(schema.payments.transactionId, transactionIdOrRef), eq(schema.payments.status, 'Awaiting Confirmation') ) });
            if (!payment) throw new Error('Payment record not found, not awaiting confirmation, or reference mismatch.');

            const updatedPayment = await tx.update(schema.payments).set({
                    status: 'Completed', updatedAt: new Date(),
                    // Use Drizzle sql helper for concatenation
                    notes: sql`COALESCE(${schema.payments.notes}, '') || ' Confirmed by admin ${sql.val(adminUserId)} on ' || NOW()`
            ...
            const processingTask: TaskInsert = {
                title: 'Process paid order', description: `${updatedPayment[0].paymentMethod} payment confirmed. Process order #${orderId} for shipping.`,
                status: 'Pending', priority: 'High', category: 'Order', relatedTo: 'Order', relatedId: orderId,
            };
            await tx.insert(schema.tasks).values(processingTask);

            const verificationTaskUpdate = await tx.update(schema.tasks).set({
                     status: 'Completed', updatedAt: new Date(),
                     notes: sql`COALESCE(${schema.tasks.notes}, '') || ' Payment confirmed by admin ${sql.val(adminUserId)} on ' || NOW()`
                 }).where(and( eq(schema.tasks.relatedTo, 'Order'), eq(schema.tasks.relatedId, orderId), eq(schema.tasks.category, 'Payment'), eq(schema.tasks.status, 'Pending') ));
            logger.info('Closed payment verification task(s)', { orderId, count: verificationTaskUpdate.rowCount });

            return { success: true, transactionId: transactionIdOrRef, status: 'Completed' as PaymentStatus, message: `${updatedPayment[0].paymentMethod} payment confirmed successfully` };
        });
        return result;
    } catch (error) {
