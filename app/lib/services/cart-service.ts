...
export interface CartSummary {
  ...
  minimumOrderRequirements: {
    retail: { met: boolean; required: number; current: number };
    wholesale: { met: boolean; required: number; current: number };
  };
}
...
export class CartService {
  ...
  async getCartItems(...args): Promise<CartSummary> {
    ...
    try {
      ...
      return {
        ...
        minimumOrderRequirements: {
          retail: { met: isRetailValid, required: this.RETAIL_MIN_UNITS, current: totalQuantity },
          wholesale: {
      ...
      logger.error('Failed to get cart items:', { userId, error });
  ...
  async updateCartItem(
    ...
    cartItemId: string,
    quantity: number
  ): Promise<CartItemSelect | { message: string }> {
    try {
  ...
  async removeCartItem(userId: string, cartItemId: string): Promise<boolean> {
    try {
      ...
      if (result.length === 0) {
        logger.warn('Attempted remove of non-existent/unauthorized item', { userId, cartItemId });
        return false;
      }
      return true;
    } catch (error) {
  ...
  async validateCart(userId: string, isWholesale: boolean = false): Promise<CartValidationResult> {
    try {
      ...
      const reqs = isWholesale
        ? minimumOrderRequirements.wholesale
        : minimumOrderRequirements.retail;
      if (!reqs.met) {
        const msg = `Order must contain at least ${reqs.required} units. Currently has ${totalQuantity}.`;
        return { isValid: false, errors: [msg], message: msg };
      }
      return { isValid: true, errors: [], message: `Order meets minimum requirements.` };
    } catch (error) {
      logger.error(`Failed to validate cart reqs:`, { userId, error });
      throw new Error('Failed to validate cart reqs.');
    }
  }
  async validateInventory(userId: string): Promise<CartValidationResult> {
    try {
      ...
      if (errors.length > 0) {
        return { isValid: false, errors, message: 'Insufficient stock for some items.' };
      }
      return { isValid: true, errors: [], message: 'All items available.' };
    } catch (error) {
      logger.error(`Failed to validate cart inventory:`, { userId, error });
  ...
  async transferCart(...args): Promise<boolean> {
    ...
    try {
      ...
      const sourceItems = await db.query.cartItems.findMany({
        where: eq(schema.cartItems.userId, fromUserId),
      });
      if (sourceItems.length === 0) return true;
      await db.transaction(async tx => {
        for (const item of sourceItems) {
          const targetItem = await tx.query.cartItems.findFirst({
            where: and(
              eq(schema.cartItems.userId, toUserId),
              eq(schema.cartItems.productVariationId, item.productVariationId)
            ),
          });
          if (targetItem) {
            ...
            await tx
              ...
              .where(eq(schema.cartItems.id, targetItem.id));
          } else {
      ...
      logger.info(`Cart transferred successfully from ${fromUserId} to ${toUserId}`);
      return true;
    } catch (error) {
      logger.error(`Failed to transfer cart:`, { fromUserId, toUserId, error });
